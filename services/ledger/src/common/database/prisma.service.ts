 ...
 * Prisma Service
 *
 * Manages database connection lifecycle for the Ledger Service.
 * Connects to ledger_db schema in the shared PostgreSQL instance.
 *
 * Best Practices:
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(private readonly secretsService: SecretsService) {
    ...
    super({
      // Suppress query logs to reduce noise
      log: [
  ...
  async onModuleInit() {
    try {
      // DATABASE_URL is already set by the factory in DatabaseModule
      // Just connect to the database
      await this.$connect();
      this.logger.log('✅ Connected to PostgreSQL (ledger_db schema)');
    } catch (error) {
