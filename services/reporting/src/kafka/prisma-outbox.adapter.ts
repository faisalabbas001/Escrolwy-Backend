...
/**
 * Prisma Outbox Adapter
 *
 * Implements the OutboxAdapter interface for the transactional outbox pattern.
 * Stores events in the outbox_events table and manages their lifecycle.
 */
@Injectable()
export class PrismaOutboxAdapter implements OutboxAdapter {
    ...
    async findPendingEvents(limit: number): Promise<OutboxEvent[]> {
      ...
      FROM reporting_db.outbox_events
