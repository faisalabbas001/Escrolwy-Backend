...
const kafka = new Kafka({
  clientId: 'test-tron-withdrawal-script',
...
/**
 * Publish ledger.external_payout_created event for Tron withdrawal
 * 
 ...
 * @param {string} destinationAddress - Tron address to send to
 * @param {string} chain - Chain identifier (default: 'trc')
 */
async function publishWithdrawalRequested(
  ...
  const event = {
    metadata: {
      ...
      eventType: topic,
      source: 'test-tron-withdrawal-script',
      version: '1.0.0',
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`
🧪 Tron Withdrawal Event Test Script

Usage:
  node test-tron-withdrawal.js [options]
...
Options:
  ...
  --destination <address>      Tron destination address (required)
  --chain <chain>              Chain identifier (default: trc)

Examples:
  # Withdraw USDT to a Tron address
  node test-tron-withdrawal.js \\
    --userId 1ea94c57-7190-4b51-9d5e-8d3d47429d90 \\
    --asset USDT \\
    --amount 10000 \\
    --destination TVox6sYqTDL2yXf2FbCpnvNPpT16ESWS7a
  ...
  # Withdraw with explicit chain (default is trc)
  node test-tron-withdrawal.js \\
    --userId 1ea94c57-7190-4b51-9d5e-8d3d47429d90 \\
    --asset USDT \\
    --amount 10000 \\
...
Notes:
  ...
  - For USDT/USDC on Tron: 6 decimals, so 10000 = 0.01 tokens
  - Chain should be 'trc' for Tron
