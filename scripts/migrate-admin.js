/**
 * Migration Helper Script - Admin Service (Local PostgreSQL)
 *
 * Creates Prisma migration file (without applying) so you can add CHECK constraints and triggers.
 * After editing, run: npm run prisma:admin:migrate:apply
 */
...
function findLatestMigration() {
  const migrationsPath = path.join(
    __dirname,
    "../services/admin/prisma/migrations"
  );
...
function runMigration() {
  try {
    ...
    console.log("📦 Creating Prisma migration for Admin service...\n");
    ...
    console.log(
      `   3. Run: npm run prisma:admin:migrate:apply\n`
    );
    console.log("=".repeat(60) + "\n");
  } catch (error) {
